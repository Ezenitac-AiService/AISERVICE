...
def request_review_page(
    ...
    429 또는 차단 안내 HTML이 반환되면 같은 요청을 점진적으로
    재시도하고, 반복 실패 시 ReviewRateLimitError를 발생시킵니다.
    """
    ...
    for attempt in range(1, MAX_429_RETRIES + 1):
        ...
        if is_rate_limited:
            ...
            print(f"서버 응답: {response_text[:1000]}")
            print(
                f"리뷰 API 요청 제한 감지 "
            ...
            if retry_after and retry_after.isdigit():
                ...
                wait_seconds = (
                    RATE_LIMIT_BASE_WAIT * attempt
                    + random.uniform(0, RATE_LIMIT_JITTER_MAX)
                )

            print(
                f"같은 요청을 재시도하기 전 "
                f"{wait_seconds:.1f}초 대기합니다."
            )
            time.sleep(wait_seconds)
            continue
