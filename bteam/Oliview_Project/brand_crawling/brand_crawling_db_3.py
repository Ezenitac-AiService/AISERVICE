...
def wait_for_cloudflare(
    page: Page,
    max_wait_seconds: int = CLOUDFLARE_WAIT_SECONDS,
) -> None:
    """
    Cloudflare 인증 화면이 나타나면 사용자가 직접 인증할 때까지 기다립니다.
    자동 인증 우회는 하지 않습니다.
    """
    ...
    print()
    print("    ⚠️ Cloudflare 또는 로봇 인증 화면이 감지되었습니다.")
    print("    ⚠️ 열린 브라우저에서 직접 인증을 완료해 주세요.")
    print(
        f"    ⚠️ 최대 {max_wait_seconds}초 동안 기다립니다."
    )
    ...
    raise RuntimeError(
        f"Cloudflare 인증이 {max_wait_seconds}초 안에 완료되지 않았습니다."
    )
...
def extract_brand_name(brand_input) -> str:
    ...
    1. data-brndnm
    2. 바로 다음 label 텍스트
    3. for 속성이 input id와 일치하는 label
    4. data-attr의 마지막 부분
    """

    # 1. input의 data-brndnm
    ...
    # 3. input id와 연결된 label
    input_id = normalize_text(
        brand_input.get("id")
    )

    if input_id:
        parent = brand_input.parent

        if parent:
            linked_label = parent.select_one(
                f'label[for="{input_id}"]'
            )

            if linked_label:
                brand_name = normalize_text(
                    linked_label.get_text(
                        " ",
                        strip=True,
                    )
                )
    ...
    # 4. data-attr의 마지막 항목
    data_attr = normalize_text(
        brand_input.get("data-attr")
    )

    if data_attr:
        parts = [
            normalize_text(part)
            for part in data_attr.split("^")
