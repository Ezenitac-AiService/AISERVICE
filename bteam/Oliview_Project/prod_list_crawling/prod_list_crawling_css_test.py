...
if __name__ == "__main__":
    ...
    # 유일한 하위 카테고리 ID 리스트 추출
    unique_category_ids = filtered_cat_df['Sub_Category_ID'].dropna().astype(str).unique().tolist()
    
    # 테스트용: 리스트 중 맨 앞의 1개 카테고리만 가져옵니다.
    test_category_ids = unique_category_ids[:1] 
    
    print(f"테스트로 {len(test_category_ids)}개의 세부 카테고리만 크롤링합니다: {test_category_ids}")
    
    # 카테고리 크롤링 진행
    crawled_data = scrape_all_categories(test_category_ids)
    
    # CSV 업데이트 (테스트용이므로 원본 대신 test_result.csv 로 별도 저장)
    update_csv(crawled_data, filename='test_result.csv')
    
    print("--- 테스트 크롤링 및 CSV 저장 자동화 작업 완료 ---")