"""
Oliview ChatA - Modernized Streamlit Web Application (06.02.app.py)
...
from graph_adapter import StreamlitGraphAdapter
...
# [7.5] Reference Review Expander Helper (Spec 037: [리뷰 N] / [제품명 리뷰 N] 일치)
def render_reference_reviews_expander(meta: Any):
    """Spec 037: 참조 리뷰 원문 및 올리브영 상품 링크 카드를 [리뷰 N] 인용 번호와 1:1 일치하여 렌더링합니다."""
    if not meta:
        return
    ref_reviews = meta.get("reference_reviews", []) if isinstance(meta, dict) else getattr(meta, "reference_reviews", [])
    if not ref_reviews:
        return
    with st.expander(f"📚 참조 리뷰 원문 ({len(ref_reviews)}건 선별)", expanded=False):
        for idx, ref in enumerate(ref_reviews, start=1):
            if isinstance(ref, dict):
                p_name = ref.get("product_name") or ref.get("clean_product_name") or "올리브영 상품"
                b_name = ref.get("brand_name") or ""
                p_url = ref.get("product_url") or ref.get("oliveyoung_search_url") or f"https://www.oliveyoung.co.kr/store/search/getSearchMain.do?query={urllib.parse.quote(p_name)}"
                r_text = ref.get("separated_sentence") or ref.get("clean_text") or ref.get("review_text") or ""
                score = ref.get("rerank_score", 0.0)
                rank = ref.get("rank", idx)
            else:
                p_name = getattr(ref, "product_name", "올리브영 상품")
                b_name = getattr(ref, "brand_name", "")
                p_url = getattr(ref, "product_url", "#")
                r_text = getattr(ref, "clean_text", "")
                score = getattr(ref, "rerank_score", 0.0)
                rank = getattr(ref, "rank", idx)

            brand_tag = f" `{b_name}`" if b_name else ""
            score_tag = f" *(관련도: {score:.2f})*" if score else ""
            st.markdown(
                f"**[리뷰 {rank}] [{p_name}]({p_url})**{brand_tag}{score_tag}  \n"
                f"> \"{r_text}\"  \n"
                f"[🛍️ 올리브영 제품 페이지 ↗]({p_url})"
            )
            if idx < len(ref_reviews):
                st.divider()
...
for message in st.session_state.chat_history:
    ...
    if role == "user":
        ...
        with st.chat_message("assistant", avatar="🤖"):
            st.markdown(content)
            render_reference_reviews_expander(meta)
...
if question_to_process:
    ...
    with st.chat_message("assistant", avatar="🤖"):
        ...
        # ── Spec 030: LangGraph StateGraph Orchestrator (Hot-swap Flag) ──
        if settings.feature_langgraph_rag:
            meta_dict = {}
            with st.status("🔍 분석 파이프라인 가동 중...", expanded=True) as status_box:
                substep_box = st.container()
                orchestrator = MultiTargetGraphOrchestrator()
                token_stream = StreamlitGraphAdapter.run_sync_stream(
                    orchestrator=orchestrator,
                    query=question_to_process,
                    session_id=st.session_state.session_id,
                    status_container=status_box,
                    substep_container=substep_box,
                    meta_dict=meta_dict,
                )
            try:
                ...
                render_reference_reviews_expander(meta_dict)
