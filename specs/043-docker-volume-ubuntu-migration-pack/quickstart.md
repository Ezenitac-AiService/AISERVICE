...
**Purpose**: Windows 개발 환경에서 마이그레이션 팩을 패키징하고 타겟 Ubuntu 24.04 LTS 서버에서 원클릭으로 복원 및 11개 엔드포인트를 검증하는 전주기 실행 매뉴얼
...
### 1.2 마이그레이션 팩 생성
Docker 볼륨 및 실사용 `.env`가 포함된 단일 아카이브(`.tar.gz`)를 생성합니다:
```powershell
python make_migration_pack.py --include-volumes --target-os ubuntu --target-cpu i7-930 --target-gpu gtx1070
```
- **출력물**: `dist/AISERVICE_Migration_Pack_<TIMESTAMP>.tar.gz` 및 `dist/checksums.sha256`

### 1.3 타겟 우분투 서버로 전송 (SCP)
```powershell
scp dist/AISERVICE_Migration_Pack_*.tar.gz ubuntu@<TARGET_UBUNTU_IP>:~/
```
...
4. MySQL DB 덤프 및 Docker 볼륨(ChromaDB/Redis) 무손실 복원
...
11. `GET http://localhost:6379` $\rightarrow$ Redis 세션/캐시 인프라 (PING-PONG)

### 3.3 검증 성공 판정
`verification_report.json`에서 `status: "PASS"` 및 `passed_endpoints: 11 / 11` 확인.
