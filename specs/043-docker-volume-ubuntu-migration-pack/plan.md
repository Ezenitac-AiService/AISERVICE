...
주요 기술적 접근:
1. **실사용 환경 변수 100% 온전한 보존**: 루트 `.env` 및 `ddns/.env`의 모든 실사용 시크릿을 번들링하여 타겟 서버에서 사용자 수동 입력 0회(Zero-Config, `chmod 600`)로 동작.
...
6. **11개 엔드포인트 E2E 헬스체크 게이트**: 복원 즉시 `verify_migration.py`를 통해 전수 검증 및 `verification_report.json` 발행.
...
**Storage**: MySQL 8.0/8.4 LTS (`pilos_v2`, `oliview_project`), ChromaDB v2 (`oliview_review_sentences_v2` with 48,210+ 1024-dim vectors), Redis 7 (AOF/RDB)  
**Testing**: `pytest`, `verify_migration.py` (11 Endpoints E2E Healthcheck Suite)  
...
**Performance Goals**: 원클릭 복원 완료 시간 $\le 25$분 (클린 OS) / $\le 10$분 (사전 준비 OS), 11개 엔드포인트 응답 성공률 100% (11/11 Pass), LLM API 지연시간 $\le 3.0$초 (Fast SLM)  
**Constraints**: Zero Manual Configuration (사용자 `.env` 수동 편집 0회), No `Illegal instruction` crashes on Non-AVX i7-930 CPU, 8GB VRAM 내 100% GPU 서빙, No Snap Docker  
**Scale/Scope**: 10개 Docker 컨테이너, 2개 MySQL DB (4.3M+ 행), 4.8만+ 벡터 임베딩, 11개 HTTP 엔드포인트, 단일 `.tar.gz` 아카이브 번들  
...
- [x] **원칙 II (Test-First & Contract Verification)**: `contracts/` 4개 명세 정의 완료, 11개 엔드포인트 E2E 계약 검증(`verify_migration.py`) 선행 정의 완료.
...
│   │   └── verify_migration.py            # [MODIFY] 11개 엔드포인트 E2E 헬스체크 게이트
...
- `step_3_build_dist_bundle`: 실사용 `.env` 및 `ddns/.env` 포함 1:1 클린 소스 복사 (캐시/빌드 폴더 제외).
- `step_4_generate_manifest`: `migration_manifest.json` v2.0 및 `checksums.sha256` 생성.
- `step_5_create_archive`: 단일 `AISERVICE_Migration_Pack_<TIMESTAMP>.tar.gz` 및 `.zip` 생성.
...
- Docker 볼륨 생성 및 물리 아카이브 복원 $\rightarrow$ Mutex 규칙으로 중복 SQL 덤프 생략.
...
- 11개 주요 엔드포인트(Nginx 80/8080, Model Gateway 8081/8090/8091, A-Team, B-Team 프론트/백엔드/챗봇, Redis) E2E 헬스체크.
...
2. **사전 검증 시뮬레이션 (Dry-Run)**:
   ...
   ```
3. **11개 엔드포인트 E2E 헬스체크**:
   ```bash
   python migration_pack/scripts/verify_migration.py
   ```

### Manual Verification
1. **Windows 패키징 검증**: `dist/`에 `.tar.gz`, `migration_manifest.json`, `checksums.sha256` 정상 생성 확인.
2. **우분투 복원 검증**: Ubuntu 24.04 환경에서 `sudo ./bootstrap_restore.sh -y` 실행 후 모든 컨테이너 `Up (healthy)` 확인.
3. **DuckDNS 갱신 검증**: `http://ezenitac.duckdns.org` 접속 및 `crontab -l` 5분 주기 등록 확인.
