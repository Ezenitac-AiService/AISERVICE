...
4. **Stage 4 (Volume & Database Mutex Restore)**:
   ...
   - 인프라 컨테이너(`pilos_db`, `bteam_db`, `redis`, `vllm-serv`) 우선 기동.
...
7. **Stage 7 (Application Staged Launch & Verification)**:
   ...
   - `verify_migration.py` 실행 $\rightarrow$ 11개 엔드포인트 E2E 헬스체크 및 `verification_report.json` 발행.

---

## 3. Exit Codes

- `0`: 부트스트랩 및 11개 엔드포인트 검증 100% 성공.
- `1`: 하드웨어 호환성 오류 또는 치명적 패키지 설치 실패.
- `2`: Docker 또는 GPU 런타임 활성화 실패.
- `3`: 볼륨/데이터베이스 복원 실패.
- `4`: 서비스 헬스체크 검증 게이트 실패 (1개 이상 엔드포인트 비정상).
