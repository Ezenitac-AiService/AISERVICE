# Implementation Plan: Audit, Zero-Hardcoding, and Decoupled Hardware Architecture (Pascal to Blackwell) OOM Hardening
...
본 계획서는 CPU와 GPU가 고정 결합되지 않고 독립적으로 조합될 수 있는 환경을 지원하기 위해, **GPU 세대별 불변 스펙(Pascal $\rightarrow$ Blackwell)**과 **CPU 명령어 세트(AVX 유무)**를 런타임에 직교적으로 실측 및 합성하는 하드웨어 감지 엔진을 구축합니다.  
...
* **독립적 GPU 아키텍처 지원 범위**:
  - **Pascal (SM 6.1)**: GTX 1070 (8GB), GTX 1080Ti (11GB) $\rightarrow$ FlashAttn 생략, Q8_0 KV Cache.
...
### Step 1: 독립적 CPU-GPU 하드웨어 룩업 테이블 및 감지 엔진 구축
* `gpu_detector.py`에 `detect_hardware_capabilities()` 구현 (CPU AVX 독립 실측 + GPU Pascal~Blackwell 독립 매칭).
