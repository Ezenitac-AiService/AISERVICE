# Tasks: Audit, Zero-Hardcoding, and 3-Axis Decoupled Hardware Evaluation Engine OOM Hardening
...
## Phase 1: Setup & 3-Axis Decoupled Hardware Specs Setup
...
## Phase 3: User Story 1 (Priority: P1) - 3대 직교 하드웨어 자율 감지 & 최적 서빙 엔진 🎯 MVP

**Goal**: GPU Compute Capability(SM), 물리 VRAM(MB), CPU 명령어(AVX) 3대 독립 축을 실측하여 최적 플래그, 모델, 컨텍스트 크기를 100% 자동 구성
...
```mermaid
flowchart TD
    Setup[Phase 1: Setup & 3-Axis Decoupled Hardware Specs] --> Foundational[Phase 2: Foundational & TDD Tests]
    Foundational --> US1[Phase 3: US1 - 3대 직교 하드웨어 자율 서빙 엔진 (MVP)]
