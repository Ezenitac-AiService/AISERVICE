# Tasks: Audit, Zero-Hardcoding, and 5-Tier GPU Architecture OOM Hardening
...
## Phase 1: Setup & 5-Tier GPU Architecture Specs Setup

**Purpose**: 5대 타겟 GPU 세대(GTX 1070, GTX 1080Ti, RTX 2080, RTX 3060, RTX 4080) 불변 스펙 룩업 테이블 및 정적 감사 스크립트 구축

- [ ] T001 전사 소스코드 및 테스트 파일 대상 하드코딩 패턴(`qwen3.5-4b`, 포트, VRAM 상수) 정적 스캔 스크립트 작성 in `model_gateway/scripts/scan_hardcoding.py`
- [ ] T002 [P] 5대 GPU 세대별 불변 스펙 룩업 테이블(`GPU_ARCHITECTURE_SPEC_TABLE`) 및 Pydantic v2 데이터 모델 정의 in `model_gateway/src/core/gpu_detector.py`
...
**Purpose**: 5대 GPU 아키텍처 자동 감지, FlashAttn/Q8/FP8 플래그 주입, Anti-Shadowing TDD 단위 테스트 선행 작성

- [ ] T004 [P] 5대 GPU(GTX 1070, 1080Ti, RTX 2080, 3060, 4080) 모의 주입 시 아키텍처 매칭 및 VRAM 사이징 단위 테스트 작성 in `model_gateway/tests/test_gpu_architecture_specs.py`
- [ ] T005 [P] Compute Capability에 따른 FlashAttention 생략(SM 6.1/7.5) 및 자동 활성화(SM 8.6/8.9) 단위 테스트 작성 in `model_gateway/tests/test_hardware_aware_flags.py`
...
## Phase 3: User Story 1 (Priority: P1) - 5대 GPU 세대별 자율 하드웨어 감지 & 최적 서빙 엔진 🎯 MVP

**Goal**: GPU Compute Capability와 물리 VRAM을 실측하여 5대 세대별 불변 체크리스트에 따라 플래그, 모델, 컨텍스트 크기를 100% 자동 구성
...
- [ ] T009 [US1] `process_manager.py`의 `build_server_command`에서 세대별 최적 플래그(SM 6.1 Q8 KV, SM 8.6+ FlashAttn, `-ngl 999`) 자동 주입 구현 in `model_gateway/src/core/process_manager.py`
...
**Checkpoint**: User Story 1 (MVP) 완결 ➔ 5대 GPU 세대별 자율 서빙 엔진 정상 작동
...
- [ ] T020 [quickstart.md](file:///c:/AISERVICE/specs/034-audit-config-oom-guards/quickstart.md)에 실시간 실행 로그 및 5대 GPU 아키텍처 실측 지표 기록 완료 in `specs/034-audit-config-oom-guards/quickstart.md`
...
```mermaid
flowchart TD
    Setup[Phase 1: Setup & 5-Tier GPU Specs Setup] --> Foundational[Phase 2: Foundational & TDD Tests]
    Foundational --> US1[Phase 3: US1 - 5대 GPU 세대별 자율 서빙 엔진 (MVP)]
