# Tasks: Audit, Zero-Hardcoding, and Decoupled Hardware Architecture (Pascal to Blackwell) OOM Hardening
...
## Phase 1: Setup & Decoupled Hardware Specs Setup

**Purpose**: 독립적 GPU 세대별(Pascal~Blackwell) 및 CPU 명령어(AVX 유무) 불변 스펙 룩업 테이블 및 정적 감사 스크립트 구축

- [ ] T001 전사 소스코드 및 테스트 파일 대상 하드코딩 패턴(`qwen3.5-4b`, 포트, VRAM 상수) 정적 스캔 스크립트 작성 in `model_gateway/scripts/scan_hardcoding.py`
- [ ] T002 [P] 독립적 GPU(Pascal~Blackwell) 및 CPU(AVX 유무) 불변 스펙 룩업 테이블 및 Pydantic v2 데이터 모델 정의 in `model_gateway/src/core/gpu_detector.py`
...
**Purpose**: 독립적 CPU-GPU 아키텍처 자동 감지, AVX 미지원 `-ngl 999` 강제, FlashAttn/Q8/FP8 플래그 주입 TDD 단위 테스트 선행 작성

- [ ] T004 [P] 임의의 CPU-GPU 조합 모의 주입 시 직교적 아키텍처 매칭 및 VRAM 사이징 단위 테스트 작성 in `model_gateway/tests/test_gpu_architecture_specs.py`
...
## Phase 3: User Story 1 (Priority: P1) - 직교적 CPU-GPU 자율 하드웨어 감지 & 최적 서빙 엔진 🎯 MVP

**Goal**: CPU 명령어 세트(AVX 유무)와 GPU Compute Capability 및 물리 VRAM을 각각 독립 실측하여 불변 체크리스트에 따라 플래그, 모델, 컨텍스트 크기를 100% 자동 구성

**Independent Test**: 다양한 CPU-GPU 조합(i7 930+GTX 1070, i7 930+RTX 3060, 최신 Intel+GTX 1070, 최신 Intel+RTX 5080)을 주입하여 `detect_hardware_capabilities()`가 반환하는 플래그와 모델이 체크리스트와 정확히 일치하는지 검증
...
**Checkpoint**: User Story 1 (MVP) 완결 ➔ 직교적 CPU-GPU 자율 서빙 엔진 정상 작동
...
- [ ] T020 [quickstart.md](file:///c:/AISERVICE/specs/034-audit-config-oom-guards/quickstart.md)에 실시간 실행 로그 및 독립 CPU-GPU 하드웨어 실측 지표 기록 완료 in `specs/034-audit-config-oom-guards/quickstart.md`
...
```mermaid
flowchart TD
    Setup[Phase 1: Setup & Decoupled Hardware Specs] --> Foundational[Phase 2: Foundational & TDD Tests]
    Foundational --> US1[Phase 3: US1 - 직교적 CPU-GPU 자율 서빙 엔진 (MVP)]
